/*
  ...
  Experimental Duino-Coin WiFi + I2C master for MKR WiFi 1010, Nano 33 IoT,
  and similar WiFiNINA boards.
*/
...
const unsigned long master_hash_budget_ms = 5;
...
const char* local_miner_name = "Arduino WiFiNINA I2C Master";
...
void sendI2CLine(byte address, String line) {
  line += "\n";
  const byte chunkSize = 31;
  for (unsigned int i = 0; i < line.length(); i += chunkSize) {
    Wire.beginTransmission(address);
    for (byte j = 0; j < chunkSize && i + j < line.length(); j++) {
      Wire.write(line[i + j]);
    }
